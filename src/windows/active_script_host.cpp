...
bool ActiveScriptHost::run(
    ...
    if (options.debug || options.break_on_entry || !session.breakpoints().all().empty()) {
        std::wstring debug_error;
        if (!debug_application.initialize(
                L"wshdbg: " + options.script_path.filename().wstring(),
                debug_error)) {
        ...
        auto* callback = new ApplicationDebugger(bridge, options.script_path);
